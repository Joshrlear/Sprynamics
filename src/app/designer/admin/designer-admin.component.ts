...
export class DesignerAdminComponent implements OnInit, AfterViewInit {
  ...
  get selection() {
    if (this.canvas) {
      return (
        this.canvas.getActiveObject() || this.canvas.getActiveObjects() || null
      )
    } else {
      return null
    }
  ...
  ngOnInit() {
    ...
    this.http
      .get(
        'https://www.googleapis.com/webfonts/v1/webfonts?sort=popularity&key=AIzaSyA-kEmBuQZhfrdS1Rije3syG3tCu8OGVcM'
      )
      .take(1)
      .subscribe(res => {
        this.fonts = res
          .json()
          .items.map(font => font.family)
          .slice(0, 100)
  ...
  refreshCanvasSize() {
    ...
    this.background.set({
      width:
        productType.width * productSpecs.dpi +
        productSpecs.bleedInches * productSpecs.dpi * 2,
      height:
        productType.height * productSpecs.dpi +
        productSpecs.bleedInches * productSpecs.dpi * 2
    })
    this.safeArea.set({
      width:
        productType.width * productSpecs.dpi -
        productSpecs.safeInches * productSpecs.dpi * 2,
      height:
        productType.height * productSpecs.dpi -
        productSpecs.safeInches * productSpecs.dpi * 2
    })
  ...
  clickNew() {
    if (
      confirm(
        'Unsaved changes will be lost. Are you sure you want to start a new template?'
      )
    ) {
  ...
  clickSave(saveNew?: boolean) {
    ...
    new Promise((resolve, reject) => {
      ...
      if (this.template.id && !saveNew) {
        this.firestore
          .update(`templates/${this.template.id}`, docData)
          .then(_ => {
      ...
      this.storage
        .putJSONNoDownloadURL(canvasData, `templates/${id}.json`)
        .then()
        .then(file => {
          ...
          if (url) {
            ...
            this.storage
              .putBase64(thumbData, `thumbnails/${id}.jpg`)
              .then(downloadURL => {
                console.log(
                  'Uploaded thumbnail to storage at URL: ' + downloadURL
                )
                this.template.thumbnail = downloadURL
                this.template.url = url
                this.firestore
                  .update(`templates/${id}`, { url, thumbnail: downloadURL })
                  .then(() => {
  ...
  shadowColorPickerChange(event) {
    ...
    this.selection.shadow.opacity = new fabric.Color(
      this.selection.shadow.color
    ).getAlpha()
    this.canvas.renderAll()
    this.saveUndo()
  }
  ...
  changeUserData() {
    if (this.userData) {
      const dataName = this.selection.textUserData
      let dataText
      ...
      if (dataName === 'name') {
        ...
        dataText =
          `${this.userData['address1']} ${this.userData['address2']}\n` +
  ...
  toggleUnderline() {
    const isUnderline =
      (this.getStyle(this.selection, 'textDecoration') || '').indexOf(
        'underline'
      ) > -1
    this.setStyle(
      this.selection,
