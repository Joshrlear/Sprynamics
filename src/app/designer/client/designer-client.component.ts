...
export class DesignerClientComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.auth.user.pipe(take(1)).subscribe((user: any) => {
      ...
      this.checkout.initOrder().then(_ => {
        ...
        const createdAgents = this.firestore.colWithIds$(
  ...
  changeAddress(address) {
    ...
    this.canvas.forEachObject(obj => {
      if (obj.isMapImage) {
        ...
        const mapImageUrl = this.gmaps.getStaticMapUrl(
          location.lat,
          location.lng,
          12,
          400,
  ...
  changeViewSide(side: 'front' | 'back') {
    ...
    const processed =
      this.template[lastSide] && this.template[lastSide].processed
  ...
  loadDesign(template: any) {
    ...
    this.viewSide = 'front'
    if (!this.template.fonts || this.template.fonts.length === 0) {
      this.template.fonts = ['Roboto']
    }
    WebFont.load({
      google: {
  ...
  processCanvas() {
    ...
    this.canvas.forEachObject(obj => {
      ...
      if (
        obj.userEditable ||
        obj.textContentType === 'data' ||
        obj.textContentType === 'property'
      ) {
  ...
  saveAndContinue() {
    ...
    canvas.width =
      (this.template.productType.width + productSpecs.bleedInches * 2) *
      productSpecs.dpi
    canvas.height =
      (this.template.productType.height + productSpecs.bleedInches * 2) *
      productSpecs.dpi
    ...
    pdfCanvas.loadFromJSON(this.template['front'], _ => {
      ...
      this.getDataURL('front', pdfCanvas).then((front: string) => {
        ...
        this.getDataURL('back', pdfCanvas).then((back: string) => {
          ...
          var img = new Image()
          const resizeImage = () => {
            var newDataUri = this.imageToDataUri(
              img,
              thumbnailSizes[this.size].width * 4,
              thumbnailSizes[this.size].height * 4
            )
              ...
              task.then(downloadURL => {
                ...
                this.checkout.updateOrder({
                  ...
                  propertyAddress: this.propertyAddress
                    ? this.propertyAddress.formatted_address
                    : ''
                })
                pdfCanvas.dispose()
                canvas.remove()
    ...
    return new Promise((resolve, reject) => {
      ...
      canvas.loadFromJSON(this.template[side], _ => {
        ...
        const offsetX =
          boundBox.left - productSpecs.bleedInches * productSpecs.dpi
        const offsetY =
          boundBox.top - productSpecs.bleedInches * productSpecs.dpi
        // console.log(offsetX, offsetY);
